...
static const char SCCS_Id[] = "@(#)makedefs.cÅ_t3.5Å_t2004/02/01";
#endif
...
#ifdef OS2
#define INCLUDE_TEMPLATE "..Å_Å_includeÅ_Å_%s"
#define SOURCE_TEMPLATE "..Å_Å_srcÅ_Å_%s"
#define DGN_TEMPLATE "..Å_Å_datÅ_Å_%s" /* where dungeon.pdf file goes */
#define DATA_TEMPLATE "..Å_Å_datÅ_Å_%s"
#define DATA_IN_TEMPLATE "..Å_Å_datÅ_Å_%s"
...
static const char
    *Dont_Edit_Code =
        "/* This source file is generated by 'makedefs'.  Do not edit. */Å_n",
    *Dont_Edit_Data =
        "#Å_tThis data file is generated by 'makedefs'.  Do not edit. Å_n";
...
#else
        printf("extended makedefs not implemented for Mac OS9Å_n");
        exit(EXIT_FAILURE);
#endif
    }
...
#endif
        && !(argv[1][0] == '-' && argv[1][1] == '-')) {
        Fprintf(stderr, "Bad arg count (%d).Å_n", argc - 1);
        (void) fflush(stderr);
        return 1;
    }
...
char *options;
{
    ...
    while (*options) {
        if (more_than_one)
            Fprintf(stderr, "makedefs -%cÅ_n", *options);
        ...
        default:
            Fprintf(stderr, "Unknown option '%c'.Å_n", *options);
            (void) fflush(stderr);
            exit(EXIT_FAILURE);
        }
        options++;
    ...
    if (more_than_one)
        Fprintf(stderr, "Completed.Å_n"); /* feedback */
}
...
char *mode;
{
    ...
    if (!rv) {
        Fprintf(stderr, "Can't open '%s'.Å_n", name);
        exit(EXIT_FAILURE);
    }
    return rv;
}
...
do_ext_makedefs(int argc, char **argv)
{
    ...
    while (argc) {
        if (argv[0][0] != '-')
            break;
        if (argv[0][1] != '-') {
            Fprintf(stderr, "Can't mix - and -- options.Å_n");
            exit(EXIT_FAILURE);
        }
...
#define CONSUME                              Å_
    argv++, argc--;                          Å_
    if (argc == 0) {                         Å_
        Fprintf(stderr, "missing optionÅ_n"); Å_
        exit(EXIT_FAILURE);                  Å_
    }
        IS_OPTION("svs")
        {
        ...
        IS_OPTION("input")
        {
            ...
            if (!strcmp(argv[0], "-")) {
                ...
                if (!inputfp) {
                    Fprintf(stderr, "Can't open '%s'.Å_n", argv[0]);
                    exit(EXIT_FAILURE);
                }
            }
            CONTINUE;
        ...
        {
            if (todo) {
                Fprintf(stderr, "Can't do grep and something else.Å_n");
                exit(EXIT_FAILURE);
            }
            todo = TODO_GREP;
            CONTINUE;
        ...
        IS_OPTION("grep-define")
        {
            ...
            if (p) {
                ...
                Fprintf(stderr, "Unknown symbol '%s'Å_n", argv[0]);
                exit(EXIT_FAILURE);
            }
            CONTINUE;
        }
...
#endif
#undef IS_OPTION
        Fprintf(stderr, "Unknown option '%s'.Å_n", argv[0]);
        exit(EXIT_FAILURE);
    }
    if (argc) {
        Fprintf(stderr, "unexpected argument '%s'.Å_n", argv[0]);
        exit(EXIT_FAILURE);
    }

    switch (todo) {
    default:
        Fprintf(stderr, "Confused about what to do?Å_n");
        exit(EXIT_FAILURE);
    case 0:
        Fprintf(stderr, "Nothing to do?Å_n");
        exit(EXIT_FAILURE);
    case TODO_GREP:
        do_grep();
        break;
...
do_grep_showvars()
{
    ...
    for (x = 0; x < SIZE(grep_vars) - 1; x++) {
        printf("%dÅ_t%sÅ_n", grep_vars[x].is_defined, grep_vars[x].name);
    }
}
...
char *id;
{
    ...
    if (rv) {
        if (grep_trace) {
            Fprintf(outputfp, "ID %d %sÅ_n", rv->is_defined, id);
        }
        return rv->is_defined;
    }

    if (grep_trace) {
        Fprintf(outputfp, "ID U %sÅ_n", id);
    }
    Fprintf(stderr, "unknown identifier '%s' in line %d.Å_n", id, grep_lineno);
    grep_errors++;
...
char *tag;
{
    ...
    Fprintf(outputfp, "%s w=%d sp=%dÅ_t", tag, grep_writing, grep_sp);
    for (x = grep_sp; x >= 0 && x > grep_sp - 6; x--) {
        Fprintf(outputfp, "[%d]=%d ", x, grep_stack[x]);
    }
    Fprintf(outputfp, "Å_n");
}
...
char *buf;
{
    ...
    case '.': /* end of if level */
        if (grep_sp == 0) {
            Fprintf(stderr, "unmatched ^. (endif) at line %d.Å_n",
                    grep_lineno);
            grep_errors++;
        } else {
    ...
    case ':': /* else */
        if (ST_ELSE & grep_stack[grep_sp]) {
            Fprintf(stderr, "multiple : for same conditional at line %d.Å_n",
                    grep_lineno);
            grep_errors++;
        }
    ...
    default: {
        ...
        Fprintf(stderr, "unknown control ^%s at line %d.Å_n", str,
                grep_lineno);
        grep_errors++;
    } break;
    }
...
{
    if (!inputfp) {
        Fprintf(stderr, "--grep requires --inputÅ_n");
    }
    if (!outputfp) {
        Fprintf(stderr, "--grep requires --outputÅ_n");
    }
    if (!inputfp || !outputfp) {
        exit(EXIT_FAILURE);
    }
...
FILE *outputfp0;
{
    ...
    while (!feof(inputfp0) && !ferror(inputfp0)) {
        ...
        if (fgets(buf, sizeof(buf), inputfp0) == 0)
            break;
        if ((tmp = strchr(buf, 'Å_n')))
            *tmp = 'Å_0';
        grep_lineno++;
        if (grep_trace) {
            Fprintf(outputfp0, "%04d %c >%sÅ_n", grep_lineno,
                    grep_writing ? ' ' : '#', buf);
        }
...
#endif
        if (grep_writing)
            Fprintf(outputfp0, "%sÅ_n", buf1);
    }
    if (ferror(inputfp0)) {
        Fprintf(stderr, "read error!Å_n");
        exit(EXIT_FAILURE);
    }
    if (ferror(outputfp0)) {
        Fprintf(stderr, "write error!Å_n");
        exit(EXIT_FAILURE);
    }
    fclose(inputfp0);
    fclose(outputfp0);
    if (grep_sp) {
        Fprintf(stderr, "%d unterminated conditional level%sÅ_n", grep_sp,
                grep_sp == 1 ? "" : "s");
        grep_errors++;
    }
    if (grep_errors) {
        Fprintf(stderr, "%d error%s detected.Å_n", grep_errors,
                grep_errors == 1 ? "" : "s");
        exit(EXIT_FAILURE);
    }
}
...
do_rumors()
{
    ...
    static const char rumors_header[] =
        "%s%04d,%06ld,%06lx;%04d,%06ld,%06lx;0,0,%06lxÅ_n";
    char tempfile[600];
    ...
    line = malloc(256);
    Sprintf(line, "rewind of Å_"%sÅ_"", tempfile);
    if (rewind(tfp) != 0) {
        perror(line);
        free(line);
        goto rumors_failure;
 ...
 *
 */
#define IGNORED_FEATURES                 Å_
    (0L | (1L << 19) /* SCORE_ON_BOTL */ Å_
     | (1L << 27)    /* ZEROCOMP */      Å_
     | (1L << 28)    /* RLECOMP */       Å_
     )
...
do_date()
{
    ...
    for (c = cbuf; *c; c++)
        if (*c == 'Å_n')
            break;
    *c = 'Å_0'; /* strip off the 'Å_n' */
    Fprintf(ofp, "#define BUILD_DATE Å_"%sÅ_"Å_n", cbuf);
    Fprintf(ofp, "#define BUILD_TIME (%ldL)Å_n", (long) clocktim);
    Fprintf(ofp, "Å_n");
#ifdef NHSTDC
    ul_sfx = "UL";
#else
    ul_sfx = "L";
#endif
    Fprintf(ofp, "#define VERSION_NUMBER 0x%08lx%sÅ_n", version.incarnation,
            ul_sfx);
    Fprintf(ofp, "#define VERSION_FEATURES 0x%08lx%sÅ_n", version.feature_set,
            ul_sfx);
#ifdef IGNORED_FEATURES
    Fprintf(ofp, "#define IGNORED_FEATURES 0x%08lx%sÅ_n",
            (unsigned long) IGNORED_FEATURES, ul_sfx);
#endif
    Fprintf(ofp, "#define VERSION_SANITY1 0x%08lx%sÅ_n", version.entity_count,
            ul_sfx);
    Fprintf(ofp, "#define VERSION_SANITY2 0x%08lx%sÅ_n", version.struct_sizes1,
            ul_sfx);
    Fprintf(ofp, "#define VERSION_SANITY3 0x%08lx%sÅ_n", version.struct_sizes2,
            ul_sfx);
    Fprintf(ofp, "Å_n");
    Fprintf(ofp, "#define VERSION_STRING Å_"%sÅ_"Å_n", version_string(buf, "."));
    Fprintf(ofp, "#define VERSION_ID Å_Å_Å_n Å_"%sÅ_"Å_n",
            version_id_string(buf, cbuf));
    Fprintf(ofp, "#define COPYRIGHT_BANNER_C Å_Å_Å_n Å_"%sÅ_"Å_n",
...
#ifdef AMIGA
    {
        ...
        Fprintf(ofp, "Å_"Å_Å_0$VER: NetHack %d.%d.%d (%d.%d.%d)Å_"Å_n",
...
windowing_sanity()
{
    ...
    Fprintf(stderr,
            "Configuration error: DEFAULT_WINDOW_SYS is not defined.Å_n");
    exit(EXIT_FAILURE);
/*NOTREACHED*/
...
#define DEFAULT_WINDOW_SYS "<undefined>"
...

    {
        ...
        if (!window_opts[i]
            ...
            Fprintf(stderr, "Configuration error: DEFAULT_WINDOW_SYS (%s)Å_n",
                    DEFAULT_WINDOW_SYS);
            Fprintf(stderr,
                    " does not match any enabled windowing system (%s%s).Å_n",
...
do_options()
{
    ...
    Fprintf(ofp,
#ifdef BETA
            "Å_n    NetHack version %d.%d.%d [beta]Å_n",
#else
            "Å_n    NetHack version %d.%d.%dÅ_n",
#endif
            VERSION_MAJOR, VERSION_MINOR, PATCHLEVEL);

    Fprintf(ofp, "Å_nOptions compiled into this edition:Å_n");
    ...
    for (i = 0; i < SIZE(build_opts); i++) {
        ...
        while (*str) {
            ...
            if (length + strlen(str) > COLNO - 5)
                Fprintf(ofp, "Å_n%s", indent), length = strlen(indent);
            else
                Fprintf(ofp, " "), length++;
    ...
    Fprintf(ofp, "Å_nÅ_nSupported windowing system%s:Å_n",
            (winsyscnt > 1) ? "s" : "");
    length = COLNO + 1; /* force 1st item onto new line */
    ...
    if (winsyscnt > 1)
        Fprintf(ofp, "Å_n%swith a default of %s.", indent, DEFAULT_WINDOW_SYS);
    Fprintf(ofp, "Å_nÅ_n");
...
do_data()
{
    ...
    Fprintf(ofp, "%s%08lxÅ_n", Dont_Edit_Data, 0L);
    ...
    while ((line = fgetline(ifp)) != 0) {
        ...
        if (*line > ' ') { /* got an entry name */
            ...
            if (line_cnt)
                Fprintf(ofp, "%dÅ_n", line_cnt), line_cnt = 0;
    ...
    if (line_cnt)
        Fprintf(ofp, "%dÅ_n", line_cnt);
    Fprintf(ofp, ".Å_n%ld,%dÅ_n", ftell(tfp), 0);
    ...
    line = malloc(256);
    Sprintf(line, "rewind of Å_"%sÅ_"", filename);
    ok = (rewind(ofp) == 0);
    if (ok) {
        Sprintf(line, "header rewrite of Å_"%sÅ_"", filename);
        ok = (fprintf(ofp, "%s%08lxÅ_n", Dont_Edit_Data,
...
static const char *special_oracle[] = {
#if 0 /*JP*/
    "Å_"...it is rather disconcerting to be confronted with the",
    ...
    "  ",
    "that P == NP.Å_"  [Garey and Johnson, p. 185.]"
#else
    "ÅuéüÇÃíËóù[Baker, Gill, and Solovay, 1975]Ç…íºñ Ç∑ÇÈÇ±Ç∆ÇÕ",
    "ÇﬁÇµÇÎç¢òfÇ∑ÇÈÇ±Ç∆Ç≈Ç†ÇÈÅD",
    "",
    "íËóù 7.18 éüÇÃÇÊÇ§Ç»çƒãAìIåæåÍ AÅCBÇ™ë∂ç›Ç∑ÇÈ",
    "  (1)  P(A) == NP(A)ÅCÇ©Ç¬",
    "  (2)  P(B) != NP(B)",
    "",
    "Ç±ÇÍÇÕåªç› P != NPÇ≈Ç†ÇÈÇ©Ç‹ÇΩÇÕ P == NPÇ≈Ç†ÇÈÇ©ÇèÿñæÇ∑ÇÈ",
    "óLå¯Ç»éËñ@Ç™Ç»Ç¢Ç±Ç∆Çã≠Ç≠é¶ÇµÇƒÇ¢ÇÈÅDÅv",
...
do_oracles()
{
    ...
    /* output a dummy header record; we'll rewind and overwrite it later */
    Fprintf(ofp, "%s%5dÅ_n", Dont_Edit_Data, 0);

    /* handle special oracle; it must come first */
    (void) fputs("---Å_n", tfp);
    offset = (unsigned long) ftell(tfp);
    Fprintf(ofp, "%05lxÅ_n", offset); /* start pos of special oracle */
    ...
    (void) fputs("---Å_n", tfp);
    offset = (unsigned long) ftell(tfp);
    Fprintf(ofp, "%05lxÅ_n", offset); /* start pos of first oracle */
    in_oracle = FALSE;
    ...
    while ((line = fgetline(ifp)) != 0) {
        ...
        if (!strncmp(line, "-----", 5)) {
            ...
            (void) fputs("---Å_n", tfp);
            offset = (unsigned long) ftell(tfp);
            Fprintf(ofp, "%05lxÅ_n", offset); /* start pos of this oracle */
        } else {
            in_oracle = TRUE;
            (void) fputs(xcrypt(line), tfp);
        }
    ...
    if (in_oracle) { /* need to terminate last oracle */
        ...
        (void) fputs("---Å_n", tfp);
        offset = (unsigned long) ftell(tfp);
        Fprintf(ofp, "%05lxÅ_n", offset); /* eof position */
    }
    ...
    if (ok) {
        Sprintf(line, "header rewrite of Å_"%sÅ_"", filename);
        ok = (fprintf(ofp, "%s%5dÅ_n", Dont_Edit_Data, oracle_cnt) >= 0);
    }
    if (ok) {
        Sprintf(line, "data rewrite of Å_"%sÅ_"", filename);
        for (i = 0; i <= oracle_cnt; i++) {
            ...
            */
            ...
            if (!(ok = (fseek(ofp, fpos, SEEK_SET) >= 0)))
                break;
            offset += txt_offset;
            if (!(ok = (fprintf(ofp, "%05lxÅ_n", offset) >= 0)))
                break;
        }
    }
    if (!ok) {
...
do_monstr()
{
    ...
    Fprintf(ofp, "%s", Dont_Edit_Code);
    Fprintf(ofp, "#include Å_"config.hÅ_"Å_n");
    Fprintf(ofp, "Å_nconst int monstr[] = {Å_n");
    for (ptr = &mons[0], j = 0; ptr->mlet; ptr++) {
        SpinCursor(3);

        i = mstrength(ptr);
        Fprintf(ofp, "%2d,%c", i, (++j & 15) ? ' ' : 'Å_n');
    }
    /* might want to insert a final 0 entry here instead of just newline */
    Fprintf(ofp, "%s};Å_n", (j & 15) ? "Å_n" : "");

    Fprintf(ofp, "Å_nvoid NDECL(monstr_init);Å_n");
    Fprintf(ofp, "Å_nvoidÅ_n");
    Fprintf(ofp, "monstr_init()Å_n");
    Fprintf(ofp, "{Å_n");
    Fprintf(ofp, "    return;Å_n");
    Fprintf(ofp, "}Å_n");
    Fprintf(ofp, "Å_n/*monstr.c*/Å_n");
...
do_permonst()
{
    ...
    Fprintf(ofp, "%s", Dont_Edit_Code);
    Fprintf(ofp, "#ifndef PM_HÅ_n#define PM_HÅ_n");

    if (strcmp(mons[0].mname, "playermon") != 0)
        Fprintf(ofp, "Å_n#defineÅ_tPM_PLAYERMONÅ_t(-1)");

    for (i = 0; mons[i].mlet; i++) {
        SpinCursor(3);

        Fprintf(ofp, "Å_n#defineÅ_tPM_");
        ...
        for (nam = c = tmpdup(mons[i].mname); *c; c++)
            if (*c >= 'a' && *c <= 'z')
                ...
                *c = '_';
        Fprintf(ofp, "%sÅ_t%d", nam, i);
    }
    Fprintf(ofp, "Å_nÅ_n#defineÅ_tNUMMONSÅ_t%dÅ_n", i);
    Fprintf(ofp, "Å_n#endif /* PM_H */Å_n");
    Fclose(ofp);
    return;
}
...
boolean parsing; /* curr_msg is valid iff this is True */
{
    ...
    if (p == summary) {
        ...
        ;    /* ignore empty [] */
             ...
             /* p points one spot past ']', usually to 'Å_n';
                we need to include the Å_n as part of the size */
        if (parsing) {
...
put_qt_hdrs()
{
     ...
     */
    if (debug)
        Fprintf(stderr, "%ld: header info.Å_n", ftell(ofp));
    ...
    for (i = 0; i < qt_hdr.n_hdr; i++) {
        if (debug)
            Fprintf(stderr, "%ld: %s header info.Å_n", ftell(ofp),
                    qt_hdr.id[i]);
...
do_objs()
{
    ...
    Fprintf(ofp, "%s", Dont_Edit_Code);
    Fprintf(ofp, "#ifndef ONAMES_HÅ_n#define ONAMES_HÅ_nÅ_n");
    ...
    for (i = 0; !i || objects[i].oc_class != ILLOBJ_CLASS; i++) {
        ...
        switch (class) {
        case WAND_CLASS:
            Fprintf(ofp, "#defineÅ_tWAN_");
            prefix = 1;
            break;
        case RING_CLASS:
            Fprintf(ofp, "#defineÅ_tRIN_");
            prefix = 1;
            break;
        case POTION_CLASS:
            Fprintf(ofp, "#defineÅ_tPOT_");
            prefix = 1;
            break;
        case SPBOOK_CLASS:
            Fprintf(ofp, "#defineÅ_tSPE_");
            prefix = 1;
            nspell++;
            break;
        case SCROLL_CLASS:
            Fprintf(ofp, "#defineÅ_tSCR_");
            prefix = 1;
            break;
        case AMULET_CLASS:
            /* avoid trouble with stupid C preprocessors */
            Fprintf(ofp, "#defineÅ_t");
            if (objects[i].oc_material == PLASTIC) {
                Fprintf(ofp, "FAKE_AMULET_OF_YENDORÅ_t%dÅ_n", i);
                prefix = -1;
                break;
            }
            break;
        case GEM_CLASS:
            ...
            if (objects[i].oc_material == GLASS) {
                Fprintf(ofp, "/* #defineÅ_t%sÅ_t%d */Å_n", objnam, i);
                prefix = -1;
                break;
            }
        default:
        ...
        if (prefix >= 0)
            Fprintf(ofp, "%sÅ_t%dÅ_n", limit(objnam, prefix), i);
        prefix = 0;
    ...
    Fprintf(ofp, "#defineÅ_tLAST_GEMÅ_t(JADE)Å_n");
    Fprintf(ofp, "#defineÅ_tMAXSPELLÅ_t%dÅ_n", nspell + 1);
    Fprintf(ofp, "#defineÅ_tNUM_OBJECTSÅ_t%dÅ_n", i);

    Fprintf(ofp, "Å_n/* Artifacts (unique objects) */Å_nÅ_n");
    ...
    for (i = 1; artifact_names[i]; i++) {
        ...
        if (!strncmp(objnam, "PLATINUM_", 9))
            objnam += 9;
        Fprintf(ofp, "#defineÅ_tART_%sÅ_t%dÅ_n", limit(objnam, 1), i);
    }

    Fprintf(ofp, "#defineÅ_tNROFARTIFACTSÅ_t%dÅ_n", i - 1);
    Fprintf(ofp, "Å_n#endif /* ONAMES_H */Å_n");
    Fclose(ofp);
    if (sumerr)
        exit(EXIT_FAILURE);
    return;
...
do_vision()
{
    ...
    Fprintf(ofp, "%s", Dont_Edit_Code);
    Fprintf(ofp, "#ifdef VISION_TABLESÅ_n");
#ifdef VISION_TABLES
    H_close_gen();
    H_far_gen();
#endif /* VISION_TABLES */
    Fprintf(ofp, "Å_n#endif /* VISION_TABLES */Å_n");
    Fclose(ofp);
    ...
    Fprintf(ofp, "#include Å_"config.hÅ_"Å_n");
    Fprintf(ofp, "#ifdef VISION_TABLESÅ_n");
    Fprintf(ofp, "#include Å_"vis_tab.hÅ_"Å_n");
    ...
    C_close_gen();
    C_far_gen();
    Fprintf(ofp, "Å_nvoid vis_tab_init() { return; }Å_n");
#endif /* VISION_TABLES */

    SpinCursor(3);

    Fprintf(ofp, "Å_n#endif /* VISION_TABLES */Å_n");
    Fprintf(ofp, "Å_n/*vis_tab.c*/Å_n");
...
H_close_gen()
{
    Fprintf(ofp, "Å_n/* Close */Å_n");
    Fprintf(ofp,
            "#define CLOSE_MAX_SB_DY %2dÅ_t/* |src row - block row| - 1Å_t*/Å_n",
            TEST_HEIGHT - 1);
    Fprintf(ofp,
            "#define CLOSE_MAX_SB_DX %2dÅ_t/* |src col - block col|Å_t*/Å_n",
            TEST_WIDTH);
    Fprintf(ofp,
            "#define CLOSE_MAX_BC_DY %2dÅ_t/* |block row - check row|Å_t*/Å_n",
            TEST_HEIGHT);
    Fprintf(ofp, "typedef struct {Å_n");
    Fprintf(ofp,
            "    unsigned char close[CLOSE_MAX_SB_DX][CLOSE_MAX_BC_DY];Å_n");
    Fprintf(ofp, "} close2d;Å_n");
    Fprintf(ofp, "extern close2d close_table[CLOSE_MAX_SB_DY];Å_n");
    return;
}
...
H_far_gen()
{
    Fprintf(ofp, "Å_n/* Far */Å_n");
    Fprintf(ofp, "#define FAR_MAX_SB_DY %2dÅ_t/* |src row - block row|Å_t*/Å_n",
            TEST_HEIGHT);
    Fprintf(ofp,
            "#define FAR_MAX_SB_DX %2dÅ_t/* |src col - block col| - 1Å_t*/Å_n",
            TEST_WIDTH - 1);
    Fprintf(ofp,
            "#define FAR_MAX_BC_DY %2dÅ_t/* |block row - check row| - 1Å_t*/Å_n",
            TEST_HEIGHT - 1);
    Fprintf(ofp, "typedef struct {Å_n");
    Fprintf(ofp, "    unsigned char far_q[FAR_MAX_SB_DX][FAR_MAX_BC_DY];Å_n");
    Fprintf(ofp, "} far2d;Å_n");
    Fprintf(ofp, "extern far2d far_table[FAR_MAX_SB_DY];Å_n");
    return;
}
...
C_close_gen()
{
    ...
    Fprintf(ofp, "Å_n#ifndef FAR_TABLE_ONLYÅ_n");
    Fprintf(ofp, "Å_nclose2d close_table[CLOSE_MAX_SB_DY] = {Å_n");
#ifndef no_vision_progress
    Fprintf(stderr, "Å_nclose:");
#endif

    for (dy = 1; dy < TEST_HEIGHT; dy++) {
        src_row = block_row + dy;
        Fprintf(ofp, "/* DY = %2d (- 1)*/Å_n  {{Å_n", dy);
#ifndef no_vision_progress
        ...
        for (dx = 0; dx < TEST_WIDTH; dx++) {
            ...
            Fprintf(ofp, "}%s", (dx < TEST_WIDTH - 1) ? ",Å_n" : "Å_n");
        }
        Fprintf(ofp, "  }},Å_n");
    }

    Fprintf(ofp, "}; /* close_table[] */Å_n"); /* closing brace for table */
    Fprintf(ofp, "#endif /* !FAR_TABLE_ONLY */Å_n");
#ifndef no_vision_progress
    Fprintf(stderr, "Å_n");
#endif
    return;
...
C_far_gen()
{
    ...
    Fprintf(ofp, "Å_n#ifndef CLOSE_TABLE_ONLYÅ_n");
    Fprintf(ofp, "Å_nfar2d far_table[FAR_MAX_SB_DY] = {Å_n");
#ifndef no_vision_progress
    Fprintf(stderr, "Å_n_far_:");
#endif

    for (dy = 0; dy < TEST_HEIGHT; dy++) {
        src_row = block_row - dy;
        Fprintf(ofp, "/* DY = %2d */Å_n  {{Å_n", dy);
#ifndef no_vision_progress
        Fprintf(stderr, " %2d", dy), (void) fflush(stderr);
#endif
    ...
    Fprintf(ofp, "}; /* far_table[] */Å_n"); /* closing brace for table */
    Fprintf(ofp, "#endif /* !CLOSE_TABLE_ONLY */Å_n");
#ifndef no_vision_progress
    Fprintf(stderr, "Å_n");
#endif
    return;
