...
#else
const char *const enc_stat[] = { "",     "¤è¤í¤á¤­", "°µÇ÷",
                                 "¸Â³¦", "²Ù½Å",     "Ä¶²á"};
#endif
...
*/
    Sprintf(nb = eos(nb), "Áá:%-1d ÂÑ:%-1d ÃÎ:%-1d ¸­:%-1d Ì¥:%-1d ",
...
*/
    Sprintf(nb = eos(newbot2), "%s:%-2ld ÂÎ:%d(%d) Ëâ:%d(%d) ³»:%-2d",
...
*/
        Sprintf(nb = eos(nb), " ÌÕÌÜ");
    if (Stunned)
/*JP
        Sprintf(nb = eos(nb), " Stun");
*/
        Sprintf(nb = eos(nb), " âÁÚô");
    if (Hallucination)
/*JP
...
*/
        Sprintf(buf, Is_astralevel(&u.uz) ? "ÀºÎî³¦ " : "ºÇ½ª»îÎý ");
    else {
