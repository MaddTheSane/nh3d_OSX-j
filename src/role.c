...
const struct Role roles[] = {
    ...
    { { "ñÏîÿêl", 0 },
      ...
      "the Duali Oasis",
#else
      "ÉfÉÖÉAÉäë∞ÇÃÉLÉÉÉìÉv",
      "ÉfÉÖÉAÉäë∞ÇÃÉIÉAÉVÉX",
#endif
      PM_BARBARIAN,
      NON_PM,
      NON_PM,
    ...
    { { "éò", 0 },
      { { "ä¯ñ{", 0 },       /* Banner Knight */
        ...
        { "óÃéÂ", 0 },        /* heads a province */
        { "ëÂñº", "çòå≥" },         /* a samurai lord */
        { "åˆâ∆", 0 },           /* Noble of the Court */
        { "åˆâ∆", "ëÂâú" } },       /* supreme commander, warlord */
      "_Amaterasu Omikami", "Raijin", "Susanowo", /* Japanese */
#endif
      "Sam",
#if 0 /*JP*/
...
/*JP:
- steed.c kick_steed() He[] Ç…ílÇ™ÉRÉsÅ[Ç≥ÇÍÇÈÇÃÇ≈í∑Ç≥ÇïœÇ¶ÇΩÇ∆Ç´Ç…ÇÕ
  äYìñïîÇ‡óvèCê≥ÅB
*/
...
#else
    { "íÅèò", "íÅèòÇÃ", "Law", ROLE_LAWFUL, A_LAWFUL },
    { "íÜóß", "íÜóßÇÃ", "Neu", ROLE_NEUTRAL, A_NEUTRAL },
    { "ç¨ì◊", "ç¨ì◊ÇÃ", "Cha", ROLE_CHAOTIC, A_CHAOTIC },
    { "ñ≥êS", "ñ≥êSÇÃ", "Una", 0, A_NONE }
#endif
};
...
*/
static char NEARDATA randomstr[] = "ÉâÉìÉ_ÉÄ";
...
int num_post_attribs;
{
    ...
    if (!post_attribs && num_post_attribs > 1)
        Strcat(buf, conjuct);
#else
    /*JP: ì˙ñ{åÍÇ≈ÇÕ "A, B, and C" Ç∆Ç¢Ç§èàóùÇÕïsóv */
...
*/
    const char *defprompt = "ìKìñÇ…ÉLÉÉÉâÉNÉ^Å[ÇëIÇÒÇ≈ÇÊÇ¢Ç≈Ç∑Ç©ÅH[ynaq] ";
...
*/
    static const char NEARDATA choosing[] = " åªç›ëIëíÜ",
/*JP
